...
describe("Data Cache", () => {
    ...
    it("Will cache data for time set in TTL and request new as time has passed", async () => {
        ...
        expect(await data.getData()).to.equal(1);
        expect(await data.getData()).to.equal(1);
        // Sleep for 10+1 ms
        ...
        expect(await data.getData()).to.equal(2);
        expect(await data.getData()).to.equal(2);
    });
    ...
    it("Update can be enforced", async () => {
        ...
        expect(await data.forceUpdate()).to.equal(2);
